...
export default function StandaloneVisitorCounter() {
  ...
  useEffect(() => {
    ...
    const countKey = 'standalone_visitor_count_v1:skills-agency';
    const seenKey = 'standalone_visitor_seen_v1:skills-agency';
    ...
    void fetch(API_URL, {
      ...
      body: JSON.stringify({ visitorId: id, appSlug: 'skills-agency' }),
    })
      .then(async (response) => {
        if (!response.ok) throw new Error('counter');
        const data = await response.json() as { appCount?: number };
        if (active && typeof data.appCount === 'number') setCount(data.appCount);
      })
      .catch(fallback);
    return () => { active = false; };
  }, []);
